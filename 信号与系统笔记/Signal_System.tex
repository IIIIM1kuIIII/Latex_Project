        ...
        $x(t)\longrightarrow \boxed{\text{系统}}\longrightarrow y(t)$
            ...
            \subsubsection{卷积积分}
                \begin{itemize}
                    \item[1)] 定义：
                        \begin{equation}
                            y(t)=f_1(t)\ast f_2(t)=\int_{-\infty}^{\infty}f_1(\tau)f_2(t-\tau)d\tau
                        \end{equation}
                    \item[2)] 性质
                    \begin{tcolorbox}[colback=blue!5!white]
                        \begin{itemize}
                            \item[(1)] $f_1(t*t_1)\ast f_2(t-t_2)=y(t-t_1-t_2)$(时移)
                            \item[(2)] $f(t)\ast \delta(t)=f(t)$ 
                        \end{itemize} 
                    \end{tcolorbox}
                \end{itemize}
            \subsubsection{连续系统的零输入响应}
                \begin{itemize}
                    \item[1)] 系统法：$y(t)=y_{zi}(t)+y_{zs}(t)$全响应=零状态+零输入($zs=zero\ stat,zi=zero\ input$)
                        \begin{itemize}
                            \item[(1)] 系统初始条件
                                $0^-$称为零输入时的初始状态，$0^+$称为加入输入后的初始状态
                        \end{itemize}  
                    \item[2)] 经典法：全响应=齐次解+特解 
                \end{itemize}
        \subsection{时不变系统}
        \subsection{因果系统}
        \subsection{稳定系统}
